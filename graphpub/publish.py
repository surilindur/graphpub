...
def publish_graph(
    ...
    """
    assert source.exists(), f"Source path does not exist: {source}"

    info(f"Publishing graph from {source}")
    ...
    while path_queue:
        ...
        if path.is_file():
            relative_path, file_ext = splitext(path.relative_to(source).as_posix())
            if file_ext in RDF_FILE_EXTENSIONS:
                try:
                    info(f"Parse {path}")
                    path_uri = URIRef(value=relative_path, base=base)
